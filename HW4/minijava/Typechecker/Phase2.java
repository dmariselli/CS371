...
/*  TODO: Things we do not understand:
    ...
    something is wrong at bottom of ex1
*/
...
//        List<Access> accesses = typechecker.getCurrentMethod().getParameterAccess();
    ...
    Stm process(APrintStmt n) {
        ...
        n.getSemi();				// yields TSemi
        Exp mem = null;
        if (exprType.getType().equals(Type.intType)) {
            mem = new CALL(new NAME(typechecker.getBuiltins().intToString),
                    new ExpList(exprType.getExpr().unEx()));
        } else {
            mem = exprType.getExpr().unEx();
        }
    ...
    ExprType process(AIdFactor n) {
        ...
        if (isGlobal) {
            return new ExprType(new Ex(new MEM(new NAME(var.getLabel()))), var.getType());
        } else {
    ...
    ExprType process(AIdLhs n) {
        ...
        Exp mem = null;
        if (isGlobal) {
            mem = new MEM(new NAME(lhsVar.getLabel()));
        } else {
            mem = lhsVar.getAccess().exp(new TEMP(typechecker.getCurrentMethod().getFrame().FP()));
        }
